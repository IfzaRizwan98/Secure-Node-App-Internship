...
module.exports = {
    ...
    idorUpdateProfile: async function (req, res, next) {
    // Week 2 Task: Enhance Authentication (Email Validation)
if (req.body.email && !validator.isEmail(req.body.email)) {
    console.log(`[AUTH-ERROR] Security Alert: Invalid email format from IP: ${req.ip}`);
    return res.status(400).json({ error: "Authentication Failed: Invalid Email Format" });
}
...
// Week 3: Security Audit Logging (The Final Audit)
console.log(`[AUDIT] Security update for User ID: ${req.user.id} at ${new Date().toISOString()}`);
// Week 2: Enhance Authentication (JWT Token Generation)
const token = jwt.sign({ id: req.user.id }, 'your_secret_key_123', { expiresIn: '1h' });
console.log(`[AUTH] Secure JWT Token generated for session verification.`);
